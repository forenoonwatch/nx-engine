...
void PhysicsMotionController::operator()(Game& game, float deltaTime) {
	game.getECS().view<Physics::BodyHandle, PlayerInputComponent,
			CameraComponent>().each([&](auto& handle, auto& pic, auto& cc) {
		Vector3f v(0.f);
